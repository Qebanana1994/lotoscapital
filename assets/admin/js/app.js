...
$('.navbar-search-field').on('input', function () {
    ...
    if (match.length == 0) {
        ...
        let data = JSON.parse($('#data-topnav').text());

        const searchValue = search; // Например, ищем значение, содержащее 11

// Функция для проверки условия частичного совпадения
        function partialMatch(item, searchValue) {
            for (const key in item) {
                if (item.hasOwnProperty(key) && Array.isArray(item[key])) {
                    for (const subItem of item[key]) {
                        if (subItem.hasOwnProperty('value') && subItem.value.toString().includes(searchValue.toString())) {
                            return true;
                        }
                    }
                }
...
// Применение фильтра с использованием частичного совпадения
        const result = data.filter(item => partialMatch(item, searchValue));
...
// Вывод результатов
        for (let i = 0; i < result.length; i++) {
            let item = result[i];
            let withdrawal = item.withdraw_information;
            $(search_result_pane).append(`
            <li>
              <a href="/admin/withdraw/details/${item.id}">${withdrawal[0].value}</a>
              <a href="/admin/withdraw/details/${item.id}" class="fw-bold text-color--3 d-block">${parseFloat(item.amount).toFixed(2)} ${item.currency}</a>
            </li>
          `);
        }
        return;
