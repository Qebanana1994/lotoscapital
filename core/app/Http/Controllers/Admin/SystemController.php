...
class SystemController extends Controller
{
    ...
    public function generateRandomUser() {
        for ($i = 0; $i < 300; $i++) {
            $randomTime = rand(0,24);
            $randomMin = rand(0,60);
            $user =  User::create([
                'username' => Str::random(10).'-test',
                'email' => Str::random(10).'@test-mail.local',
                'ev' => 1,//Email verified
                'password' => Hash::make(Str::random(25)),
                'created_at' => Carbon::now()->modify('-'.$randomTime.' hours')->minute($randomMin)->format('Y-m-d H:i:s'),
            ]);

            $this->generateDeposit($user);

        }


        dd('stop here. 300 Users created');
    }
    public function generateDeposit(user $user){
        $dep = rand(1,100).'000';
        $userWallet = $user;
        $userWallet->deposit_wallet += $dep;
        $userWallet->save();
        ...
        $hyip->invest($dep, $wallet, '');
        $invest = Invest::where('user_id', $user->id)->first();
        $times = Carbon::now()->modify('-1 day')->hour(rand(0,24))->minute(rand(0,60))->format('Y-m-d H:i:s');
        $invest->next_time = $times;
        $invest->last_time = null;
        $invest->save();

    }
    public function systemUpdate() {
        $pageTitle = 'System Updates';
        //$invests = Invest::with('plan.timeSetting', 'user')->where('status', 1)->where('next_time', '<=', Carbon::now())->orderBy('last_time')->take(100)->get();
