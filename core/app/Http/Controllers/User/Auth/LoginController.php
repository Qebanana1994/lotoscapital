...
class LoginController extends Controller
{
    ...
    public function logout()
    {
        ...
        $notify[] = ['success', 'Успешный выход из системы'];
    ...
    public function socialLogin(Request $request, $provider)
    {
        ...
        $params = \App\Models\Extension::where('name', $provider)->first()->shortcode;
        $redirect = url('user/slogin/' . $provider . '/callback');
        $config = new \SocialiteProviders\Manager\Config($params->client_id->value, $params->client_secret->value, $redirect);

        $socialUser = Socialite::driver($provider)->setConfig($config)->user();
    ...
    {
        $socialAccount = SocialLogin::where('provider', $provider)
