...
class InvestController extends Controller
{
    ...
    public function statistics()
    {
        ...
        $invests    = Deposit::where('user_id', auth()->id())->orderBy('id', 'desc')->paginate(getPaginate(10));
        $activePlan = Deposit::where('user_id', auth()->id())->where('status', 1)->count();

        $investChart = Deposit::where('user_id', auth()->id())->get();
