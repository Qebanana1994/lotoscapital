...
<div class="dashboard-inner">
    <div class="row justify-content-center">
        <div class="col-md-8">
            <div class="mb-4">
                <h3 class="mb-2">Вывод средств</h3>
                <p>Вывод средст осуществляется в срок от 15 минут до 48 часов.</p>
            </div>
            <div class="text-end mb-4">
                <a href="{{ route('user.withdraw.history') }}" class="btn btn--secondary btn--smd"><i class="las la-long-arrow-alt-left"></i> История выплат</a>
            </div>
            ...
            <div class="card custom--card">
                ...
                <div class="card-body">
                    <form action="{{route('user.withdraw.money')}}" method="post">
                        @csrf
<h4 class="mb-2">Доступный баланс {{ showAmount(auth()->user()->interest_wallet) }} {{ $general->cur_text }}</h4>
                        ...
                        <div class="mt-3 preview-details d-none">
                            <ul class="list-group text-center">
                                <li class="list-group-item d-flex justify-content-between">
                                    <span>@lang('Лимиты')</span>
                                    <span><span class="min fw-bold">0</span> {{__($general->cur_text)}} - <span class="max fw-bold">0</span> {{__($general->cur_text)}}</span>
                                </li>
                                <li class="list-group-item d-flex justify-content-between">
                                    <span>@lang('Комиссия')</span>
                                    <span><span class="charge fw-bold">0</span> {{__($general->cur_text)}}</span>
                                </li>
                                <li class="list-group-item d-flex justify-content-between">
                                    <span>@lang('К получению')</span> <span><span class="receivable fw-bold"> 0</span> {{__($general->cur_text)}} </span>
                                </li>
                                ...
                                <li class="list-group-item d-none justify-content-between in-site-cur">
                                    <span>В <span class="base-currency"></span></span>
    ...
    (function ($) {
        ...
        $('select[name=method_code]').change(function(){
            ...
            if (resource.currency != '{{ $general->cur_text }}') {
                var rateElement = `<span>Курс конвертации</span> <span class="fw-bold">1 {{__($general->cur_text)}} = <span class="rate">${rate}</span>  <span class="base-currency">${resource.currency}</span></span>`;
