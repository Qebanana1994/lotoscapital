...
//        Route::post('login/metamask/verify', 'metamaskLoginVerify')->name('login.metamask.verify');
        ...
        Route::get('slogin/{social}/callback', 'socialLogin')->name('login.social.callback');

        Route::get('slogin/{social}', function ($social) {
            $params = \App\Models\Extension::where('name', $social)->first()?->shortcode;
            if (!$params) {
                return abort(404);
            }

            $redirect = url('user/slogin/'.$social.'/callback');
            $config = new \SocialiteProviders\Manager\Config($params->client_id->value,$params->client_secret->value, $redirect);
            return Socialite::driver($social)->setConfig($config)->redirect();
        });
